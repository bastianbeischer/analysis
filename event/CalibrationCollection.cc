...
CalibrationCollection::CalibrationCollection(const CalibrationCollection& other) :
  TObject(other)
{
  for (std::map<unsigned short, TH1S*>::const_iterator it = other.m_pedestalHistograms.begin(); it != other.m_pedestalHistograms.end(); it++)
    m_pedestalHistograms[it->first] = new TH1S(*(it->second));
  for (std::map<unsigned short, TH1S*>::const_iterator it = other.m_ledHistograms.begin(); it != other.m_ledHistograms.end(); it++)
    m_ledHistograms[it->first] = new TH1S(*(it->second));
}
...
CalibrationCollection::~CalibrationCollection()
{
  for (std::map<unsigned short, TH1S*>::iterator it = m_pedestalHistograms.begin(); it != m_pedestalHistograms.end(); it++)
    delete it->second;
  for (std::map<unsigned short, TH1S*>::iterator it = m_ledHistograms.begin(); it != m_ledHistograms.end(); it++)
    delete it->second;
}

const TH1S* CalibrationCollection::pedestalHistogram(unsigned short detId) const
{
  std::map<unsigned short, TH1S*>::const_iterator it = m_pedestalHistograms.find(detId);
...
void CalibrationCollection::addPedestalHistogram(unsigned short detId)
{
  if (!m_pedestalHistograms[detId]) {
    ...
    m_pedestalHistograms[detId] = new TH1S(title, title, 4096, 0, 4096);
  }
}

void CalibrationCollection::addPedestalValue(unsigned short detId, int value)
{
  TH1S* hist = m_pedestalHistograms[detId];
  assert(hist);
  hist->Fill(value);
}
...
std::vector<unsigned short> CalibrationCollection::pedestalIds() const
{
  ...
  std::map<unsigned short, TH1S*>::const_iterator mapIt = m_pedestalHistograms.begin();
  std::map<unsigned short, TH1S*>::const_iterator mapEnd = m_pedestalHistograms.end();
  while (mapIt != mapEnd) {
...
const TH1S* CalibrationCollection::ledHistogram(unsigned short detId) const
{
  std::map<unsigned short, TH1S*>::const_iterator it = m_ledHistograms.find(detId);
...
void CalibrationCollection::addLedHistogram(unsigned short detId)
{
  if (!m_ledHistograms[detId]) {
    ...
    m_ledHistograms[detId] = new TH1S(title, title, 4096, 0, 4096);
  }
}

void CalibrationCollection::addLedValue(unsigned short detId, int value)
{
  TH1S* hist = m_ledHistograms[detId];
  assert(hist);
  hist->Fill(value);
}
...
std::vector<unsigned short> CalibrationCollection::ledIds() const
{
  ...
  std::vector<unsigned short>::iterator idVectorIt = idVector.begin();
  std::map<unsigned short, TH1S*>::const_iterator mapIt = m_ledHistograms.begin();
  std::map<unsigned short, TH1S*>::const_iterator mapEnd = m_ledHistograms.end();
  while (mapIt != mapEnd) {
    *idVectorIt = mapIt->first;
    ++idVectorIt;
    ++mapIt;
