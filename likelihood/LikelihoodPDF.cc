...
LikelihoodPDF::LikelihoodPDF(const Likelihood* likelihood, const KineticVariable& variable)
  : TF1(qPrintable(Enums::label(likelihood->likelihoodVariableType()) + " " + Enums::label(variable.particle())),
    this, &LikelihoodPDF::rootFunctionPointer, likelihood->measuredValueMin(), likelihood->measuredValueMax(), 0,
    "LikelihoodPDF", "rootFunctionPointer")
  , m_parameters(likelihood->defaultParameters())
  , m_variable(variable)
...
{
  return m_likelihood->eval(x[0], Hypothesis(m_variable.particle(), m_variable.curvature()), m_parameters);
}
