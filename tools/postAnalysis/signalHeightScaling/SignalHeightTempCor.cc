...
QMap<unsigned short, TF1> SignalHeightTempCor::s_SignalHeightTempCorFits;
...
, m_graph(0)
{
  ...
  TF1& f = s_SignalHeightTempCorFits[sipmId];
  addFunction(&f);
...
void SignalHeightTempCor::fit(unsigned short sipmId, TH2D* histogram) 
{
  ...
  //Linear fit
  QString htitle =QString("Fit signal height temperature dependent 0x%1").arg(sipmId,0,16);
  TF1 f(qPrintable(htitle), "pol1");
  f.SetLineColor(kRed);
  f.SetLineWidth(1);
  f.SetLineStyle(2);
  graph.Fit(&f, "EQ");
  s_SignalHeightTempCorFits.insert(sipmId, f);
}

void SignalHeightTempCor::save(unsigned short sipmId) 
{
  QList<QVariant> param;
  const TF1& f = s_SignalHeightTempCorFits[sipmId];
...
void SignalHeightTempCor::save()
{
  save(m_sipmId);
}

void SignalHeightTempCor::saveAll()
{
  foreach (unsigned int sipmId, s_SignalHeightTempCorFits.keys()) {
    save(sipmId);
  }
}
